...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="grid gap-4">
      ...
      {/* Resumo */}
      <Resumo orcamentoId={id} />
    </div>
  );
}

async function Resumo({ orcamentoId }: { orcamentoId: string }) {
  const { total, bdi, total_com_bdi } = await calculateOrcamento(orcamentoId);
  return (
